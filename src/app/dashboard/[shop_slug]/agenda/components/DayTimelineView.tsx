    ...
    <div className="glass-card-dark overflow-hidden">
      <div className="overflow-x-auto custom-scrollbar">
        <div className="min-w-[720px]">
          {/* Header row */}
          <div
            className="grid border-b border-white/10 sticky top-0 z-20 bg-zinc-950/80 backdrop-blur-xl"
            style={{ gridTemplateColumns: `72px repeat(${professionals.length}, minmax(200px, 1fr))` }}
          >
            <div className="px-3 py-4 text-[11px] font-bold uppercase tracking-widest text-zinc-500">
              Hora
            </div>
            {professionals.map((prof) => {
              ...
              return (
                <div
                  key={prof.id}
                  className="flex items-center gap-2 px-4 py-4 border-l border-white/5"
                >
                  <span
                    className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold text-white ${color.dot}`}
                  >
          ...
          >
            ...
            <div className="relative" style={{ height: gridHeight }}>
              {hourMarks.map((m) => (
                <div
                  key={m}
                  className="absolute left-0 right-0 px-3 text-xs text-zinc-500 -translate-y-1/2"
            ...
            {professionals.map((prof) => {
              ...
              const profAppointments = appointments.filter(
                (a) => a.professional_id === prof.id && a.status !== 'cancelado'
              )
                      ...
                      {timedExceptions.map((exc) => {
                        ...
                        return (
                          <div
                            ...
                            className="absolute left-1 right-1 rounded-lg border border-dashed border-zinc-600 bg-zinc-800/40 px-2 py-1 text-zinc-400 hover:bg-zinc-800/70 hover:text-zinc-200 transition-colors overflow-hidden"
                            style={{ top, height }}
                          >
                      ...
                      {profAppointments.map((apt) => {
                        ...
                        return (
                          <button
                            ...
                            className={`absolute rounded-lg border-l-4 px-2 py-1 text-left overflow-hidden transition-all hover:z-10 hover:shadow-lg ${color.border} ${
                              isFinalized ? 'bg-zinc-800/40 opacity-60' : color.bg
