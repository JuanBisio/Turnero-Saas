-- FIX BOOKING SLOTS & SERVICES (v5 - DYNAMIC SCHEDULES)
-- 1. ADDS dynamic schedule lookup from 'schedules' table.
-- 2. Respects Day of Week (0=Sun, 6=Sat).
-- 3. Handles "Day Off" (no schedule = no slots).
-- 4. Includes all previous fixes (Service Name, Robust Duration, Safe Columns).
...
BEGIN
  ...
  -- Get Services (Safe: no is_active check)
...
-- =============================================
-- PART 2: FIX AVAILABLE SLOTS (DYNAMIC)
-- =============================================
...
BEGIN
  ...
  -- 3. Determine Schedule (Dynamic Logic)
  v_dow := EXTRACT(DOW FROM p_date)::INTEGER; -- 0 = Sunday, 6 = Saturday

  IF v_professional_id IS NOT NULL THEN
    -- Specific Professional Schedule
    SELECT start_time, end_time INTO r_start, r_end
    FROM schedules
    WHERE professional_id = v_professional_id
    AND day_of_week = v_dow;

    -- If no schedule found, return empty (Day Off)
    IF r_start IS NULL OR r_end IS NULL THEN
      RETURN;
    END IF;

  ELSE
    -- "Anyone" / Shop Wide: Find min start and max end of ALL active pros for this day
  ...
  WHILE curr + v_interval <= r_end LOOP
    ...
    IF NOT EXISTS (
      ...
      -- If specific pro, check only their appts. If 'Any', check if *everyone* is blocked? NO.
      -- Simplified: If 'Any', we optimistically show slot if NO GLOBAL BLOCK exists?
      -- Actually, correctly handling "Any" requires complex logic (is AT LEAST ONE pro free).
      -- For now, reusing strict overlap check:
