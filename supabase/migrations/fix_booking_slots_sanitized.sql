-- FIX BOOKING SLOTS & SERVICES (CONSOLIDATED - SANITIZED - v3)
...
BEGIN
  ...
  -- 3. Get Services (Assuming all services in table are active as column is_active is missing)
  SELECT string_agg(name || ' (' || duration_minutes || 'm)', ', ') INTO v_services
  FROM services
  WHERE shop_id = v_shop_id;
  -- removed is_active check for services
